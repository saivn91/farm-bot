...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.settings_panel = SettingsPanel(
            self.tabs.tab("Cai dat"),
            self.instances,
            self.settings,
            on_save=self._on_save,
            on_add=self._on_add,
            on_remove=self._on_remove,
            on_test_adb=self._on_test_adb,
        )
    ...
    def _on_start(self, inst_id: int):
        ...
        if inst is None or inst.is_running:
            return
        engine = FarmEngine(
            inst,
            on_log=lambda msg: self.after(0, lambda: self.log_panel.append(inst_id, msg)),
        )
    ...
    def _on_test_adb(self, inst_id: int) -> str:
        inst = self._get(inst_id)
        if inst is None:
            return "Khong tim thay instance."
        adb = make_adb(inst.adb_path, inst.emu_index)
        if inst.adb_serial:
            adb.serial = inst.adb_serial
